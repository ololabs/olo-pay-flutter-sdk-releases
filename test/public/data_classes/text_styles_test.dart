...
void main() {
  final allParamsMap =
      const TextStyles(
        ...
        errorFontAsset: "error font asset",
      ).toMap();
  ...
  group('TextStyles:', () {
    group('merge():', () {
      group('textColor:', () {
        ...
        test('With otherStyles, uses otherStyles.textColor', () {
          ...
          final newStyles = TextStyles.merge(
      ...
      group('errorTextColor:', () {
        test('No theme, uses defaultLightThemeErrorTextColor', () {
          final newStyles = TextStyles.merge(theme: null);
          expect(
            newStyles.errorTextColor,
            const Color.fromRGBO(196, 45, 50, 1),
          );
        });
    ...
    group('errorFontAsset: ', () {
      test('With otherStyles, uses otherStyles.errorFontAsset', () {
        const otherStyles = TextStyles.only(errorFontAsset: "font");
        final newStyles = TextStyles.merge(
          otherStyles: otherStyles,
          theme: null,
        );

        expect(newStyles.errorFontAsset, otherStyles.errorFontAsset);
      });
    });
    ...
    group('toMap():', () {
      group('All Params:', () {
        ...
        test('Has correct keys', () {
          ...
          expect(allParamsMap.containsKey("errorTextSize"), true);
          expect(allParamsMap.containsKey("fontAsset"), true);
          expect(allParamsMap.containsKey("fontName"), true);
          expect(allParamsMap.containsKey("errorFontAsset"), true);
        });
        ...
        test('Keys have correct values', () {
          ...
          expect(allParamsMap["errorTextSize"], 8.0);
          expect(allParamsMap["fontAsset"], "font asset");
          expect(allParamsMap["fontName"], "font name");
          expect(allParamsMap["errorFontAsset"], "error font asset");
        });
      });

      group('Empty Params:', () {
        test('Has correct length', () {
          expect(emptyParamsMap.length, 9);
        });
        ...
        test('Has correct keys', () {
          ...
          expect(emptyParamsMap.containsKey("errorTextSize"), true);
          expect(allParamsMap.containsKey("fontAsset"), true);
          expect(allParamsMap.containsKey("fontName"), true);
          expect(allParamsMap.containsKey("errorFontAsset"), true);
        });
        ...
        test('Keys have correct values', () {
          ...
          expect(emptyParamsMap["errorTextSize"], null);
          expect(emptyParamsMap["fontAsset"], null);
          expect(emptyParamsMap["fontName"], null);
          expect(emptyParamsMap["errorFontAsset"], null);
        });
      });
    });
  });
