...
class TextStyles {
  ...
  /// Default font size of the error message below the input widget _**(Android [CardDetailsFormTextField] only)**_
  ///
  /// **Important:** Default values are only used in conjunction with [TextStyles.merge]
  static const defaultErrorFontSize = 8.0;
  ...
  ///
  /// **Important:** Default values are only used in conjunction with [TextStyles.merge]
  static const defaultDarkThemeErrorTextColor = Color.fromRGBO(
    219,
    129,
    132,
    1,
  ...
  /// Convenience property for getting a [TextStyles] instance with light theme
  /// defaults for the [CardDetailsFormTextField] widget
  static const TextStyles lightFormDefaults = TextStyles.only(
    textColor: Colors.black,
    hintTextColor: Colors.grey,
  );

  /// Convenience property for getting a [TextStyles] instance with dark theme
  /// defaults for the [CardDetailsFormTextField] widget
  static const TextStyles darkFormDefaults = TextStyles.only(
  ...
  ///
  /// **Note:** Only available for the [CardDetailsFormTextField] widget on Android
  ...
  /// Font asset for the error message text below the input widget _**(Android [CardDetailsFormTextField] only)**_
  ///
  /// The path to a font asset defined in pubspec.yaml. If not specified, default platform-specific system fonts
  /// will be used.
  ///
  ...
  /// - [TextStyles.errorTextSize]
  ///   1. Attempts to use `theme.textTheme.bodySmall.fontSize` first
  ///   1. If either [theme] or the above theme value are `null` it falls back to a size of `8`
  ///
  ...
  factory TextStyles.merge({
    ...
    return TextStyles(
      textColor:
          otherStyles?.textColor ??
          theme?.textTheme.bodyMedium?.color ??
          theme?.colorScheme.onSurface ??
          defaultTextColor,
      errorTextColor:
          otherStyles?.errorTextColor ??
          theme?.inputDecorationTheme.errorStyle?.color ??
          theme?.colorScheme.error ??
          defaultErrorTextColor,
      cursorColor:
          otherStyles?.cursorColor ??
          theme?.textSelectionTheme.cursorColor ??
          theme?.colorScheme.primary ??
          defaultCursorColor,
      hintTextColor:
          otherStyles?.hintTextColor ??
          theme?.inputDecorationTheme.hintStyle?.color ??
          theme?.hintColor ??
          defaultHintTextColor,
      textSize:
          otherStyles?.textSize ??
          theme?.textTheme.bodyMedium?.fontSize ??
          defaultFontSize,
      errorTextSize:
          otherStyles?.errorTextSize ??
          theme?.textTheme.bodySmall?.fontSize ??
          defaultErrorFontSize,
      fontAsset: otherStyles?.fontAsset,
      iOSFontName: otherStyles?.iOSFontName,
      errorFontAsset: otherStyles?.errorFontAsset,
    );
  }
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      DataKeys.errorTextSizeKey: errorTextSize,
