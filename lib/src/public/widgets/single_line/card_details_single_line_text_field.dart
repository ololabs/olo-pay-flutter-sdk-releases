...
///       // Use paymentMethod to submit an order to [Olo's Ordering API](https://developer.olo.com/docs/load/olopay#section/Submitting-a-Basket-via-the-Ordering-API)
///     }
...
class CardDetailsSingleLineTextField extends StatefulWidget {
  ...
  ///
  /// The [constraints] propety can be used to set the height of the native views.
  ///
  ...
  /// An object to provide custom hints for fields of this widget
  final Hints hints;
...
class _CardDetailsSingleLineTextFieldState
  ...
  Future<void> platformViewCreatedCallback(id) async {
    ...
    _controller =
        CardDetailsSingleLineTextFieldController(_channel!, onControllerError);
  ...
  void focusChangedHandler(MethodCall call) {
    final args = call.arguments as Map<dynamic, dynamic>;
    final bool isValid = args[DataKeys.isValidKey];
    final CardField? focusedField =
        CardField.fromStringValue(args[DataKeys.focusedFieldParameterKey]);
    final Map<dynamic, dynamic> fieldStateArgs =
        args[DataKeys.fieldStatesParameterKey];

    widget.onFocusChanged
        ?.call(focusedField, isValid, fieldStateArgs.toFieldStateMap());
  }
